...
def unit_con(unit=None, raw_data=None, d1=None, d2=None):
    if unit == 'MJ/tonnes' or unit == 'kgCO2/tonnes':
        mj_kg = raw_data * 0.001
        mj_m3 = mj_kg * d1
        mj_m2 = mj_m3 * d2
        return mj_m2
    elif unit == 'MJ/kg' or unit == 'kgCO2/kg':
        mj_m3 = raw_data * d1
        mj_m2 = mj_m3 * d2
        return mj_m2
    elif unit == 'MJ/m3' or unit == 'kgCO2/m3':
        mj_m2 = raw_data * d1
        return mj_m2
...
try:
    layout = [
        ...
        [sg.Combo(LEE_a, size=(30, 1), enable_events=True, key='-COMBO2-', default_value=LEE_a[0])],

        [sg.Text('Data: '), sg.InputText(size=(25, 1), key='data')],

        [sg.Text('Units:'), sg.Combo(LEE_u, size=(23, 1), enable_events=True, key='-COMBO3-', default_value=LEE_u[2])],
        [sg.Text('Unit Data 1: ', visible=False, key='data_t1'), sg.InputText(size=(22, 1), key='data_u1', visible=False)],
        [sg.Text('Unit Data 2: ', visible=False, key='data_t2'), sg.InputText(size=(20, 1), key='data_u2', visible=False)],

        [sg.Button("Submit")],
        [sg.InputText(size=(32, 1), key='c_unit')],
        [sg.Text('Score:'), sg.InputText(size=(25, 1), key='score')],
        [sg.Button("Exit")]
    ]
        ...
        try:
            ...
            if event == "Exit" or event == sg.WIN_CLOSED:
                ...
                if val_val[0] == 'Low embodied energy':
                    item = values[event]
                    window['-COMBO2-'].update(value=LEE_a[0], values=LEE_a)
                    window['data'].update("")
                    window['-COMBO3-'].update(value=LEE_u[2], values=LEE_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                elif val_val[0] == 'Low embodied Carbon':
                    item = values[event]
                    window['-COMBO2-'].update(value=LEC_a[0], values=LEC_a)
                    window['data'].update("")
                    window['-COMBO3-'].update(value=LEC_u[2], values=LEC_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                    ...
                    window['data'].update("")
                    window['-COMBO3-'].update(value=DURABLE_u[0], values=DURABLE_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                    ...
                    window['data'].update("")
                    window['-COMBO3-'].update(value=THERMAL_u[1], values=THERMAL_u) # CHECK AS BOTH U AND R ARE USED
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                    ...
                    window['data'].update("")
                    window['-COMBO3-'].update(value=ACOUSTIC_u[0], values=ACOUSTIC_u[0:0])
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                    ...
                    window['-COMBO3-'].update(value=FR_u[0], values=FR_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                    ...
                    item = values[event]
                    window['-COMBO2-'].update(value=MOIST_a[2], values=MOIST_a)
                    window['data'].update("")
                    window['-COMBO3-'].update(value=MOIST_u[1], values=MOIST_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                    ...
                    window['data'].update("")
                    window['-COMBO3-'].update(value=RC_u[0], values=RC_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False, value='')
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False, value='')
                ...
                if val_val[1] == 'Noise Reduction Coefficient':
                    window['-COMBO3-'].update(value=ACOUSTIC_u[1], values=ACOUSTIC_u[1:1])
                elif val_val[1] == 'db Reduction wall/floor unit' or val_val[1] == 'db Reduction insulation, Windows and doors' or val_val[1] == 'Impact absorption':
                    window['-COMBO3-'].update(value=ACOUSTIC_u[0], values=ACOUSTIC_u[0:0])
                elif val_val[1] == 'U value for windows':
                    window['-COMBO3-'].update(value="U-Value (W/m2K)", values=["U-Value (W/m2K)"])
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                elif val_val[1] == 'insulation':
                    window['-COMBO3-'].update(value=THERMAL_u[1], values=THERMAL_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                elif val_val[1] == 'Floor Finish and others':
                    window['-COMBO3-'].update(value=THERMAL_u[1], values=THERMAL_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                elif val_val[1] == 'Floor Finish and others' or val_val[1] == 'insulation':
                    window['-COMBO3-'].update(value=THERMAL_u[1], values=THERMAL_u)
                elif val_val[1] == 'paint SD':
                    window['-COMBO3-'].update(value="SD", values=["SD"])
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                elif val_val[1] == 'paint MVTR':
                    window['-COMBO3-'].update(value="MVTR", values=["MVTR"])
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                elif val_val[1] == 'other BMs':
                    window['-COMBO3-'].update(value=MOIST_u[1], values=MOIST_u)
                    window['data_t1'].update(visible=False)
                    window['data_u1'].update(visible=False)
                    window['data_t2'].update(visible=False)
                ...
                if val_val[3] == 'MJ/m3':
                    window['data_t1'].update(value='Thickness: ',visible=True)
                    window['data_u1'].update(visible=True)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                    print("HERE")
                elif val_val[3] == 'MJ/kg':
                    window['data_t1'].update(value='Density: ', visible=True)
                    window['data_u1'].update(visible=True)
                    window['data_t2'].update(value='Thickness: ', visible=True)
                    window['data_u2'].update(visible=True)
                elif val_val[3] == 'MJ/tonnes':
                    window['data_t1'].update(value='Density: ', visible=True)
                    window['data_u1'].update(visible=True)
                    window['data_t2'].update(value='Thickness: ', visible=True)
                    window['data_u2'].update(visible=True)

                elif val_val[3] == 'kgCO2/m3':
                    window['data_t1'].update(value='Thickness: ', visible=True)
                    window['data_u1'].update(visible=True)
                    window['data_t2'].update(visible=False)
                    window['data_u2'].update(visible=False)
                elif val_val[3] == 'kgCO2/kg':
                    window['data_t1'].update(value='Density: ', visible=True)
                    window['data_u1'].update(visible=True)
                    window['data_t2'].update(value='Thickness: ', visible=True)
                    window['data_u2'].update(visible=True)
                elif val_val[3] == 'kgCO2/tonnes':
                    window['data_t1'].update(value='Density: ', visible=True)
                    window['data_u1'].update(visible=True)
                    window['data_t2'].update(value='Thickness: ', visible=True)
                    window['data_u2'].update(visible=True)
                ...
                print(event, values)
                if val_val[4] != '' and val_val[5] != '':
                    converted_data = unit_con(val_val[3], float(val_val[2]), float(val_val[4]), float(val_val[5]))
                elif val_val[4] != '':
                    converted_data = unit_con(val_val[3], float(val_val[2]), float(val_val[4]))
                else:
                    converted_data = val_val[2]
                score = score_cal(val_val[0], val_val[1], converted_data)
                window['c_unit'].update(str(converted_data))
                window['score'].update(score)

                window['data_u1'].update(value='')
                window['data_u2'].update(value='')
